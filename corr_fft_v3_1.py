...
def compute_w(sig_len):
	...
	for i in range(0, sig_len):
		for j in range(1, sig_len):
			W_t = np.power(np.power(np.e, -1j * np.pi / j), i)
			W[i][j] = np.sign(W_t.real) * np.power(2,np.round(np.log2(abs(W_t.real)))) + (np.sign(W_t.imag) * np.power(2,np.round(np.log2(abs(W_t.imag)))))*1j
			W_I_t = np.power(np.power(np.e, 1j * np.pi / j), i)
			W_I[i][j] = np.sign(W_I_t.real) * np.power(2,np.round(np.log2(abs(W_I_t.real)))) + (np.sign(W_I_t.imag) * np.power(2,np.round(np.log2(abs(W_I_t.imag)))))*1j
			# W[i][j] = (i,j)
